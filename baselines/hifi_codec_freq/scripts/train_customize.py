...
def parse_args():
    ...
    parser.add_argument("--wb_exp_name", type=str, default="DAC_FREQ")
...
def load(
    ...
    discriminator = dac.model.Discriminator(
        rates=args.Discriminator.rates,
        periods=args.Discriminator.periods,
        fft_sizes=args.Discriminator.fft_sizes,
        sample_rate=args.Discriminator.sample_rate,
        bands=args.Discriminator.bands,
    )
    print("Generator num Params: ", sum(p.numel() for p in generator.parameters() if p.requires_grad))
    print("Discriminator num Params: ", sum(p.numel() for p in discriminator.parameters() if p.requires_grad))
    ...
    optimizer_d = torch.optim.AdamW(discriminator.parameters(), 
                                    lr=args.AdamW.lr, betas=args.AdamW.betas,)
    scheduler_d = torch.optim.lr_scheduler.ExponentialLR(optimizer_d, args.ExponentialLR.gamma)
    ...
    gan_loss = dac.nn.loss.GANLoss(discriminator)
...
def train_loop(state, batch, accel, lambdas):
    ...
    output["other/batch_size"] = signal.batch_size * accel.world_siz
...
def train_dns(
    ...
    if accel.local_rank == 0:
        wandb.login(key="e2922251e1696c0689d10b7d657d058deff4b7f1")
...
def main(args):
    print(f"Reproducing Experiment: DAC16kHz on DNSCHALLENGE\nConfiguration:{args.wb_exp_name}")
